...
# Minimum move before starting a element dragging
MIN_MOVE = 5
...
# Distance where wire is automativally attached to a connector
ANCHOR_DISTANCE = 5
POINT_RADIUS = 2                        # Radius of the point of wire ends
# Minimum size of the endling segments of a wire
MIN_SEGMENT_SIZE = 2

# LD constants
LD_LINE_SIZE = 40                       # Distance between two lines in a ladder rung
# Size (width, height) of a ladder element (contact or coil)
LD_ELEMENT_SIZE = (21, 15)
...
# Distance (x, y) between each comment and rung of the ladder
LD_OFFSET = (10, 10)
...
# Default size of the interval between two divergence branches
SFC_DEFAULT_SEQUENCE_INTERVAL = 40
# Size of extra lines for simultaneous divergence and convergence
SFC_SIMULTANEOUS_SEQUENCE_EXTRA = 20
...
[COIL_NORMAL, COIL_REVERSE, COIL_SET, COIL_RESET,
    COIL_RISING, COIL_FALLING] = range(6)
[SELECTION_DIVERGENCE, SELECTION_CONVERGENCE,
    SIMULTANEOUS_DIVERGENCE, SIMULTANEOUS_CONVERGENCE] = range(4)

# Constants for defining the type of dragging that has been selected
[HANDLE_MOVE, HANDLE_RESIZE, HANDLE_POINT,
    HANDLE_SEGMENT, HANDLE_CONNECTOR] = range(5)

# List of value for resize handle that are valid
VALID_HANDLES = [(1, 1), (1, 2), (1, 3), (2, 3),
                 (3, 3), (3, 2), (3, 1), (2, 1)]
...
[ALIGN_LEFT, ALIGN_CENTER, ALIGN_RIGHT, ALIGN_TOP,
    ALIGN_MIDDLE, ALIGN_BOTTOM] = range(6)
...
ERROR_HIGHLIGHT = (wxpatch.HashableColour(255, 255, 0),
                   wxpatch.HashableColour(wx.RED))
SEARCH_RESULT_HIGHLIGHT = (wxpatch.HashableColour(
    255, 165, 0), wxpatch.HashableColour(wx.WHITE))
...
def round_scaling(x, n, constraint=0) -> int:
    fraction = x / n
    if constraint == -1:
        xround = int(fraction)
    else:
...
def DrawHighlightedText(dc, text, highlights, x, y):
    ...
    for start, end, highlight_type in highlights:
        ...
        wxpatch.DrawRectangle(dc, x + offset_width, y, part_width, part_height)
...
class Graphic_Element(ToolTipProducer):
    ...
    def __init__(self, parent, id=None):
        ...
        self.Pos = wxpatch.Point(0, 0)
        self.Size = wx.Size(0, 0)
        self.BoundingBox = wxpatch.Rect(0, 0, 0, 0)
        self.Visible = False
    ...
    def SetBestSize(self, scaling, x_factor=0.5, y_factor=0.5) -> tuple[int, int]:
        width, height = self.GetSize()
        posx, posy = self.GetPosition()
        min_width, min_height = self.GetMinSize()
        if width < min_width:
            self.Pos.x = max(
                0, int(self.Pos.x - (width - min_width) * x_factor))
            width = min_width
        if height < min_height:
            self.Pos.y = max(
                0, int(self.Pos.y - (height - min_height) * y_factor))
            height = min_height
    ...
    def HitTest(self, pt, connectors=True):
        if connectors:
            ...
            rect = wxpatch.Rect(self.Pos.x, self.Pos.y,
    ...
    def GetRedrawRect(self, movex=0, movey=0):
        ...
        return wxpatch.Rect(
            self.BoundingBox.x - int(HANDLE_SIZE / scalex) - 3 - abs(movex),
            self.BoundingBox.y -
            int(HANDLE_SIZE / scaley) - 3 - abs(movey),
            self.BoundingBox.width + 2 *
            (int(HANDLE_SIZE / scalex) + abs(movex) + 1) + 4,
            self.BoundingBox.height + 2 * (int(HANDLE_SIZE / scaley) + abs(movey) + 1) + 4)

    def Refresh(self, rect=None):
        if self.Visible:
            if rect is not None:
                self.Parent.RefreshRect(
                    self.Parent.GetScrolledRect(rect), False)
            else:
                self.Parent.RefreshRect(
                    self.Parent.GetScrolledRect(self.GetRedrawRect()), False)
    ...
    def TestHandle(self, event):
        ...
        pt = wxpatch.Point(*self.Parent.CalcUnscrolledPosition(pos.x, pos.y))

        left = (self.BoundingBox.x - 2) * scalex - HANDLE_SIZE
        center = (self.BoundingBox.x + self.BoundingBox.width //
                  2) * scalex - HANDLE_SIZE // 2
        right = (self.BoundingBox.x + self.BoundingBox.width + 2) * scalex

        top = (self.BoundingBox.y - 2) * scaley - HANDLE_SIZE
        middle = (self.BoundingBox.y + self.BoundingBox.height / 2) * \
            scaley - HANDLE_SIZE // 2
        bottom = (self.BoundingBox.y + self.BoundingBox.height + 2) * scaley

        extern_rect = wxpatch.Rect(
            left, top, right + HANDLE_SIZE - left, bottom + HANDLE_SIZE - top)
        intern_rect = wxpatch.Rect(left + HANDLE_SIZE, top + HANDLE_SIZE,
                                   right - left - HANDLE_SIZE, bottom - top - HANDLE_SIZE)
    ...
    def OnLeftDown(self, event, dc, scaling):
        ...
        self.StartPos = wxpatch.Point(self.Pos.x, self.Pos.y)
        self.CurrentDrag = wxpatch.Point(0, 0)
    ...
    def OnMotion(self, event, dc, scaling):
        ...
        if event.Dragging() and self.oldPos:
            ...
            if self.Dragging:
                dragx, dragy = self.ProcessDragging(
                    movex, movey, event, scaling)
    ...
    def ProcessDragging(self, movex, movey, event, scaling, width_fac=1, height_fac=1):
        ...
        if handle_type == HANDLE_RESIZE:
            ...
            if handle[1] == 1:
                ...
                if scaling is not None:
                    movey = -(round_scaling(height -
                              movey, scaling[1]) - height)
                y = movey
                if event.ShiftDown():
                    height -= 2 * movey
                else:
        ...
        # If it is a move handle, Move this element
            ...
            movey = max(-self.BoundingBox.y, movey)
            if scaling is not None:
                movex = round_scaling(self.Pos.x + movex,
                                      scaling[0]) - self.Pos.x
                movey = round_scaling(self.Pos.y + movey,
                                      scaling[1]) - self.Pos.y
    ...
    def DrawHighlightment(self, dc):
        ...
        wxpatch.DrawRectangle(dc, int(round((self.Pos.x - 1) * scalex)) - 2,
    ...
    def Draw(self, dc):
        if not getattr(dc, "printing", False):
            ...
            if self.Selected:
                ...
                center = (self.BoundingBox.x + self.BoundingBox.width //
                          2) * scalex - HANDLE_SIZE // 2
                right = (self.BoundingBox.x +
                         self.BoundingBox.width + 2) * scalex

                top = (self.BoundingBox.y - 2) * scaley - HANDLE_SIZE
                middle = (self.BoundingBox.y + self.BoundingBox.height //
                          2) * scaley - HANDLE_SIZE // 2
                bottom = (self.BoundingBox.y +
                          self.BoundingBox.height + 2) * scaley

                for x, y in [(left, top), (center, top), (right, top),
                             (left, middle), (right, middle),
                             (left, bottom), (center, bottom), (right, bottom)]:
                    wxpatch.DrawRectangle(dc, x, y, HANDLE_SIZE, HANDLE_SIZE)
...
class Graphic_Group(Graphic_Element):
        ...
        for element in self.Elements:
            if isinstance(element, Wire):
                ...
                if pos is not None:
                    ...
                    new_pos = wxpatch.Point(x + dx, y + dy)
    ...
    def RefreshBoundingBox(self):
        if len(self.Elements) > 0:
            ...
            self.BoundingBox = wxpatch.Rect(
                minx, miny, maxx - minx, maxy - miny)
        else:
            self.BoundingBox = wxpatch.Rect(0, 0, 0, 0)
        self.Pos = wxpatch.Point(self.BoundingBox.x, self.BoundingBox.y)
    ...
    def OnLeftDown(self, event, dc, scaling):
        ...
        self.StartPos = wxpatch.Point(*self.GetPosition(True))
        for element in self.Elements:
...
class Connector(DebugDataConsumer, ToolTipProducer):
    ...
    def GetRedrawRect(self, movex=0, movey=0):
        ...
        x = min(parent_pos[0] + self.Pos.x, parent_pos[0] +
                self.Pos.x + self.Direction[0] * CONNECTOR_SIZE)
        y = min(parent_pos[1] + self.Pos.y, parent_pos[1] +
                self.Pos.y + self.Direction[1] * CONNECTOR_SIZE)
        ...
        if self.Direction[1] == 0:
            ...
            if self.Edge == "rising" and self.Direction[1] == 1:
                y -= 5
                height += 5
        rect = wxpatch.Rect(x - abs(movex), y - abs(movey),
                            width + 2 * abs(movex), height + 2 * abs(movey))
        if self.ValueSize is None and isinstance(self.ComputedValue, str):
            self.ValueSize = self.ParentBlock.Parent.GetMiniTextExtent(
                self.ComputedValue)
    ...
    def Clone(self, parent=None):
        ...
        return Connector(parent, self.Name, self.Type, wxpatch.Point(self.Pos[0], self.Pos[1]),
        ...
        if len(self.Wires) > 0:
            ...
            for wire, index in self.Wires:
                if (exclude is None) or (wire not in exclude):
                    if index == 0:
                        wire.MoveStartPoint(wxpatch.Point(x, y))
                    else:
                        wire.MoveEndPoint(wxpatch.Point(x, y))
    ...
    def TestPoint(self, pt, direction=None, exclude=True):
        inside = False
        check_point = (not exclude) and (
            direction is None or self.Direction == direction)

        if check_point:
            # Calculate a square around the end point of this connector
            parent_pos = self.ParentBlock.GetPosition()
            x = parent_pos[0] + self.Pos.x + \
                self.Direction[0] * CONNECTOR_SIZE - ANCHOR_DISTANCE
            y = parent_pos[1] + self.Pos.y + \
                self.Direction[1] * CONNECTOR_SIZE - ANCHOR_DISTANCE
            width = ANCHOR_DISTANCE * 2 + \
                abs(self.Direction[0]) * CONNECTOR_SIZE
            height = ANCHOR_DISTANCE * 2 + \
                abs(self.Direction[1]) * CONNECTOR_SIZE
            rect = wxpatch.Rect(x, y, width, height)
    ...
    def DrawHighlightment(self, dc):
        ...
        wxpatch.DrawLine(dc, round((xstart + self.Direction[0]) * scalex), round((ystart + self.Direction[1]) * scaley),
    ...
    def Draw(self, dc):
        ...
        if self.Negated:
            ...
            xcenter = parent_pos[0] + self.Pos.x + \
                (CONNECTOR_SIZE * self.Direction[0]) // 2
            ycenter = parent_pos[1] + self.Pos.y + \
                (CONNECTOR_SIZE * self.Direction[1]) // 2
            wxpatch.DrawCircle(dc, xcenter, ycenter, CONNECTOR_SIZE // 2)
        else:
            xstart = parent_pos[0] + self.Pos.x
            ystart = parent_pos[1] + self.Pos.y
            if self.Edge == "rising":
                # If connector has a rising edge, draw a right arrow
                wxpatch.DrawLine(dc, xstart, ystart, xstart - 4, ystart - 4)
                wxpatch.DrawLine(dc, xstart, ystart, xstart - 4, ystart + 4)
            ...
            if self.Selected:
                ...
                wxpatch.DrawLine(dc, 
                    xstart + 2 * self.Direction[0], ystart + 2 * self.Direction[1], xend, yend)
            else:
                xend = xstart + CONNECTOR_SIZE * self.Direction[0]
                yend = ystart + CONNECTOR_SIZE * self.Direction[1]
                wxpatch.DrawLine(dc, 
                    xstart + self.Direction[0], ystart + self.Direction[1], xend, yend)
        if self.Direction[0] != 0:
...
class Wire(Graphic_Element, DebugDataConsumer):
    ...
    def GetRedrawRect(self, movex=0, movey=0):
        ...
        if self.ValueSize is not None:
            ...
            if self.BoundingBox[2] > width * 4 or self.BoundingBox[3] > height * 4:
                ...
                rect = rect.Union(wxpatch.Rect(x, y, width, height))
                ...
                x = (self.Points[middle].x +
                     self.Points[middle + 1].x - width) // 2
                if self.BoundingBox[3] > height and self.Segments[middle] in [NORTH, SOUTH]:
                    y = (self.Points[middle].y +
                         self.Points[middle + 1].y - height) // 2
                else:
    ...
    def Clone(self, parent, connectors=None, dx=0, dy=0):
        ...
        if start_connector is not None and end_connector is not None:
            wire = Wire(parent)
            wire.SetPoints([(point.x + dx, point.y + dy)
                           for point in self.Points])
            start_connector.Connect((wire, 0), False)
            end_connector.Connect((wire, -1), False)
            wire.ConnectStartPoint(
                start_connector.GetPosition(), start_connector)
                ...
                if 0 < idx < len(self.Points) - 1:
                    ...
                    if idx == 1:
                        if self.Segments[0][0] == 0:
                            ...
                            movex = round_scaling(
                                self.Points[0].x + MIN_SEGMENT_SIZE * self.Segments[0][0], scaling[0], self.Segments[0][0]) - point.x
                        if self.Segments[0][1] == 0:
                            movey = 0
                        elif (point.y + movey - self.Points[0].y) * self.Segments[0][1] < MIN_SEGMENT_SIZE:
                            movey = round_scaling(
                                self.Points[0].y + MIN_SEGMENT_SIZE * self.Segments[0][1], scaling[0], self.Segments[0][1]) - point.y
                    elif idx == len(self.Points) - 2:
                        if self.Segments[-1][0] == 0:
                            movex = 0
                        elif (self.Points[-1].x - (point.x + movex)) * self.Segments[-1][0] < MIN_SEGMENT_SIZE:
                            movex = round_scaling(
                                self.Points[-1].x + MIN_SEGMENT_SIZE * self.Segments[0][0], scaling[0], self.Segments[0][0]) - point.x
                        if self.Segments[-1][1] == 0:
                            movey = 0
                        elif (self.Points[-1].y - (point.y + movey)) * self.Segments[-1][1] < MIN_SEGMENT_SIZE:
                            movey = round_scaling(
                                self.Points[-1].y - MIN_SEGMENT_SIZE * self.Segments[-1][1], scaling[1], -self.Segments[-1][1]) - point.y
    ...
    def RefreshBoundingBox(self):
        if len(self.Points) > 0:
            ...
            if len(self.Points) > 1:
                minx, minbbxx = min(
                    minx, self.Points[-1].x), min(minbbxx, self.Points[-1].x - end_radius)
                maxx, maxbbxx = max(
                    maxx, self.Points[-1].x), max(maxbbxx, self.Points[-1].x + end_radius)
                miny, minbbxy = min(
                    miny, self.Points[-1].y), min(minbbxy, self.Points[-1].y - end_radius)
                maxy, maxbbxy = max(
                    maxy, self.Points[-1].y), max(maxbbxy, self.Points[-1].y + end_radius)
            self.Pos.x, self.Pos.y = minx, miny
            self.Size = wx.Size(maxx - minx, maxy - miny)
            self.BoundingBox = wxpatch.Rect(
                minbbxx, minbbxy, maxbbxx - minbbxx + 1, maxbbxy - minbbxy + 1)
    ...
    def RefreshRealPoints(self):
        if len(self.Points) > 0:
            ...
            for point in self.Points:
                self.RealPoints.append(
                    [float(point.x - self.Pos.x), float(point.y - self.Pos.y)])
        ...
        for i in range(len(self.Points) - 1):
            ...
            if i == len(self.Points) - 2 and self.EndConnected is not None:
                x2 = self.Points[i + 1].x + \
                    self.Segments[-1][0] * CONNECTOR_SIZE
                y2 = self.Points[i + 1].y + \
                    self.Segments[-1][1] * CONNECTOR_SIZE
            else:
                x2, y2 = self.Points[i + 1].x, self.Points[i + 1].y
            # Calculate a rectangle around the segment
            rect = wxpatch.Rect(min(x1, x2) - ANCHOR_DISTANCE, min(y1, y2) - ANCHOR_DISTANCE,
    ...
    def TestPoint(self, pt):
        ...
        rect = wxpatch.Rect(self.Points[0].x - ANCHOR_DISTANCE, self.Points[0].y - ANCHOR_DISTANCE,
                            2 * ANCHOR_DISTANCE, 2 * ANCHOR_DISTANCE)
        if rect.Contains(pt.x, pt.y):
            return 0
        # Test the wire end point
        if len(self.Points) > 1:
            rect = wxpatch.Rect(self.Points[-1].x - ANCHOR_DISTANCE, self.Points[-1].y - ANCHOR_DISTANCE,
                                2 * ANCHOR_DISTANCE, 2 * ANCHOR_DISTANCE)
            if rect.Contains(pt.x, pt.y):
                return -1
        return None
    ...
    def SetPoints(self, points, verify=True):
        if len(points) > 1:
            self.Points = [wxpatch.Point(x, y) for x, y in points]
            # Calculate the start and end directions
            self.StartPoint = [None, vector(self.Points[0], self.Points[1])]
            self.EndPoint = [None, vector(self.Points[-1], self.Points[-2])]
            # Calculate the start and end points
            self.StartPoint[0] = wxpatch.Point(self.Points[0].x + CONNECTOR_SIZE * self.StartPoint[1][0],
                                               self.Points[0].y + CONNECTOR_SIZE * self.StartPoint[1][1])
            self.EndPoint[0] = wxpatch.Point(self.Points[-1].x + CONNECTOR_SIZE * self.EndPoint[1][0],
                   ...
                   self.Segments[-1] == vector(self.Points[i + 1], self.Points[i + 2]):
                    ...
                    if i < len(self.Points) - 2:
                        ...
                        if next == segment or is_null_vector(add_vectors(segment, next)):
                            self.Points.insert(
                                i + 1, wxpatch.Point(self.Points[i + 1].x, self.Points[i + 1].y))
    ...
    def GetPoints(self, invert=False):
        ...
        points[0] = wxpatch.Point(points[0].x - CONNECTOR_SIZE * self.StartPoint[1][0],
                                  points[0].y - CONNECTOR_SIZE * self.StartPoint[1][1])
        points[-1] = wxpatch.Point(points[-1].x - CONNECTOR_SIZE * self.EndPoint[1][0],
    ...
    def GeneratePoints(self, realpoints=True):
        ...
        end = wxpatch.Point(self.EndPoint[0].x + self.EndPoint[1][0] * MIN_SEGMENT_SIZE,
                            self.EndPoint[0].y + self.EndPoint[1][1] * MIN_SEGMENT_SIZE)
        start = wxpatch.Point(self.StartPoint[0].x + self.StartPoint[1][0] * MIN_SEGMENT_SIZE,
            ...
            if i + 1 == len(self.Points) - 1:
                ...
                if i == 0:
                    ...
                    if v_end != self.Segments[0] or v_end == self.EndPoint[1]:
                        self.Points.insert(1, wxpatch.Point(start.x, start.y))
                ...
                # The current point is the second
                    ...
                    if product(v_end, self.Segments[0]) < 0:
                        self.Points.insert(2, wxpatch.Point(
                    ...
                    if product(self.Segments[i - 1], self.EndPoint[1]) == 0:
                        ...
                        if product(v_end, self.EndPoint[1]) <= 0:
                            ...
                            if i == 2:
                                ...
                                if self.Segments[1][1] != 0:
                                    test_point.x = end.x
                                vector_test = vector(
                                    self.Points[0], test_point, False)
                                test = norm(vector_test) > MIN_SEGMENT_SIZE and product(
                                    self.Segments[0], vector_test) > 0
                            ...
                            if test:
                                ...
                                self.Segments[i] = (-self.EndPoint[1]
                                                    [0], -self.EndPoint[1][1])
                            else:
                                ...
                                if self.Segments[1][0] != 0:
                                    self.Points[2].x = (
                                        self.Points[1].x + end.x) // 2
                                if self.Segments[1][1] != 0:
                                    self.Points[2].y = (
                                        self.Points[1].y + end.y) // 2
                                self.Points.insert(3, wxpatch.Point(
                        ...
                        if product(self.Segments[i - 1], self.EndPoint[1]) < 0:
                            ...
                            if self.Segments[i - 1][0] != 0:
                                self.Points[i].x = (
                                    end.x + self.Points[i - 1].x) // 2
                            if self.Segments[i - 1][1] != 0:
                                self.Points[i].y = (
                                    end.y + self.Points[i - 1].y) // 2
                        # A point is added
                        self.Points.insert(
                            i + 1, wxpatch.Point(self.Points[i].x, self.Points[i].y))
                        self.Segments.insert(
                            i + 1,
        ...
        if len(self.Points) > 1:
            ...
            for i, point in enumerate(self.RealPoints):
                ...
                if not (i == 0 and self.StartConnected) and not (i == len(self.Points) - 1 and self.EndConnected):
                    ...
                    self.Points[i] = wxpatch.Point(
                        minx + x + pointx, miny + y + pointy)
            self.StartPoint[0] = self.Points[0]
            self.EndPoint[0] = self.Points[-1]
                ...
                if not (i == 0 and self.StartConnected) and not (i == len(self.Points) - 1 and self.EndConnected):
                    ...
                    realpointy = max(-dir[1] * MIN_SEGMENT_SIZE,
                                     min(int(round(point[1])),
                                         height - dir[1] * MIN_SEGMENT_SIZE))
                    self.Points[i] = wxpatch.Point(
                        minx + x + realpointx, miny + y + realpointy)
            self.StartPoint[0] = self.Points[0]
            self.EndPoint[0] = self.Points[-1]
            self.GeneratePoints(False)
    ...
    def RotateStartPoint(self):
        self.SetStartPointDirection(
            (self.StartPoint[1][1], -self.StartPoint[1][0]))
    ...
    def MoveSegment(self, idx, movex, movey, scaling):
        if 0 < idx < len(self.Segments) - 1:
            if self.Segments[idx] in (NORTH, SOUTH):
                ...
                if scaling is not None:
                    movex = round_scaling(
                        self.Points[idx].x + movex, scaling[0]) - self.Points[idx].x
                    if idx == 1 and (self.Points[1].x + movex - self.Points[0].x) * self.Segments[0][0] < MIN_SEGMENT_SIZE:
                        movex = round_scaling(
                            self.Points[0].x + MIN_SEGMENT_SIZE * self.Segments[0][0], scaling[0], self.Segments[0][0]) - self.Points[idx].x
                    elif idx == len(self.Segments) - 2 and (self.Points[-1].x - (self.Points[-2].x + movex)) * self.Segments[-1][0] < MIN_SEGMENT_SIZE:
                        movex = round_scaling(
                            self.Points[-1].x - MIN_SEGMENT_SIZE * self.Segments[-1][0], scaling[0], -self.Segments[-1][0]) - self.Points[idx].x
                ...
                if scaling is not None:
                    movey = round_scaling(
                        self.Points[idx].y + movey, scaling[1]) - self.Points[idx].y
                    if idx == 1 and (self.Points[1].y + movey - self.Points[0].y) * self.Segments[0][1] < MIN_SEGMENT_SIZE:
                        movex = round_scaling(
                            self.Points[0].y + MIN_SEGMENT_SIZE * self.Segments[0][1], scaling[0], self.Segments[0][1]) - self.Points[idx].y
                    elif idx == len(self.Segments) - 2 and (self.Points[-1].y - (self.Points[-2].y + movey)) * self.Segments[-1][1] < MIN_SEGMENT_SIZE:
                        movey = round_scaling(
                            self.Points[idx].y - MIN_SEGMENT_SIZE * self.Segments[-1][1], scaling[1], -self.Segments[-1][1]) - self.Points[idx].y
    ...
    def AddSegment(self):
        ...
        if handle_type == HANDLE_SEGMENT:
            ...
            if len(self.Segments) > 1:
                ...
                if dir[0] != 0:
                    pointx = (self.Points[segment].x +
                              self.Points[segment + 1].x) // 2
                if dir[1] != 0:
                    pointy = (self.Points[segment].y +
                              self.Points[segment + 1].y) // 2
                self.Points.insert(segment + 1, wxpatch.Point(pointx, pointy))
                self.Segments.insert(segment + 1, (dir[1], dir[0]))
                self.Points.insert(segment + 2, wxpatch.Point(pointx, pointy))
                self.Segments.insert(segment + 2, dir)
            else:
                p1x = p2x = self.Points[segment].x
                p1y = p2y = self.Points[segment].y
                if dir[0] != 0:
                    p1x = (2 * self.Points[segment].x +
                           self.Points[segment + 1].x) // 3
                    p2x = (self.Points[segment].x + 2 *
                           self.Points[segment + 1].x) // 3
                if dir[1] != 0:
                    p1y = (2 * self.Points[segment].y +
                           self.Points[segment + 1].y) // 3
                    p2y = (self.Points[segment].y + 2 *
                           self.Points[segment + 1].y) // 3
                self.Points.insert(segment + 1, wxpatch.Point(p1x, p1y))
                self.Segments.insert(segment + 1, (dir[1], dir[0]))
                self.Points.insert(segment + 2, wxpatch.Point(p1x, p1y))
                self.Segments.insert(segment + 2, dir)
                self.Points.insert(segment + 3, wxpatch.Point(p2x, p2y))
                self.Segments.insert(segment + 3, (dir[1], dir[0]))
                self.Points.insert(segment + 4, wxpatch.Point(p2x, p2y))
        ...
        if event.ControlDown():
            ...
            if direction in [(EAST, WEST), (WEST, EAST)]:
                ...
                if self.StartConnected is not None:
                    ...
                    self.MoveStartPoint(wxpatch.Point(
                        self.StartPoint[0].x, avgy))
                if self.EndConnected is not None:
                    ...
                    self.MoveEndPoint(wxpatch.Point(self.EndPoint[0].x, avgy))
                self.Parent.RefreshBuffer()
                ...
                if self.EndConnected is not None:
                    ...
                    self.MoveEndPoint(wxpatch.Point(avgx, self.EndPoint[0].y))
                self.Parent.RefreshBuffer()
        else:
            self.ResetPoints()
            self.GeneratePoints()
    ...
    def ProcessDragging(self, movex, movey, event, scaling):
        ...
        if handle_type == HANDLE_POINT:
            ...
            if scaling is not None:
                movex = round_scaling(
                    self.Points[handle].x + movex, scaling[0]) - self.Points[handle].x
                movey = round_scaling(
                    self.Points[handle].y + movey, scaling[1]) - self.Points[handle].y
            # Try to connect point to a connector
            new_pos = wxpatch.Point(
                self.Points[handle].x + movex, self.Points[handle].y + movey)
            connector = self.Parent.FindBlockConnector(
                new_pos, self.GetConnectionDirection())
            if connector:
    ...
    def DrawHighlightment(self, dc):
        ...
        if len(self.Points) > 0 and (not self.StartConnected or self.OverStart):
            wxpatch.DrawCircle(dc, round(self.Points[0].x * scalex),
                          round(self.Points[0].y * scaley),
                          (POINT_RADIUS + 1) * scalex + 2)
        if len(self.Points) > 1 and (not self.EndConnected or self.OverEnd):
            wxpatch.DrawCircle(dc, 
                self.Points[-1].x * scalex, self.Points[-1].y * scaley, (POINT_RADIUS + 1) * scalex + 2)
        # Draw the wire lines and the last point (it seems that DrawLines stop before the last point)
        if len(self.Points) > 1:
            points = [wxpatch.Point(round((self.Points[0].x - self.Segments[0][0]) * scalex),
                                    round((self.Points[0].y - self.Segments[0][1]) * scaley))]
            points.extend([wxpatch.Point(round(point.x * scalex),
                          round(point.y * scaley)) for point in self.Points[1:-1]])
            points.append(wxpatch.Point(round((self.Points[-1].x + self.Segments[-1][0]) * scalex),
    ...
    def Draw(self, dc):
        ...
        # Draw the start and end points if they are not connected or the mouse is over them
        if len(self.Points) > 0 and (not self.StartConnected or self.OverStart):
            wxpatch.DrawCircle(dc, self.Points[0].x, self.Points[0].y, POINT_RADIUS)
        if len(self.Points) > 1 and (not self.EndConnected or self.OverEnd):
            wxpatch.DrawCircle(dc, self.Points[-1].x, self.Points[-1].y, POINT_RADIUS)
        # Draw the wire lines and the last point (it seems that DrawLines stop before the last point)
        if len(self.Points) > 1:
            points = [wxpatch.Point(
                self.Points[0].x - self.Segments[0][0], self.Points[0].y - self.Segments[0][1])]
            points.extend([point for point in self.Points[1:-1]])
            points.append(wxpatch.Point(
                self.Points[-1].x + self.Segments[-1][0], self.Points[-1].y + self.Segments[-1][1]))
        else:
            points = []
        dc.DrawLines(points)
        ...
        if not getattr(dc, "printing", False) and self.SelectedSegment is not None:
            ...
            wxpatch.DrawLine(dc, self.Points[self.SelectedSegment].x - 1, self.Points[self.SelectedSegment].y,
                        self.Points[self.SelectedSegment + 1].x + end, self.Points[self.SelectedSegment + 1].y)
        if self.Value is not None and not isinstance(self.Value, bool) and self.Value != "undefined":
            dc.SetFont(self.Parent.GetMiniFont())
            dc.SetTextForeground(wx.NamedColour("purple"))
            if self.ValueSize is None and isinstance(self.ComputedValue, str):
                self.ValueSize = self.Parent.GetMiniTextExtent(
                    self.ComputedValue)
            if self.ValueSize is not None:
                width, height = self.ValueSize
                if self.BoundingBox[2] > width * 4 or self.BoundingBox[3] > height * 4:
                    x = self.Points[0].x + width * \
                        (self.StartPoint[1][0] - 1) // 2
                    y = self.Points[0].y + height * (self.StartPoint[1][1] - 1)
                    dc.DrawText(self.ComputedValue, x, y)
                    x = self.Points[-1].x + width * \
                        (self.EndPoint[1][0] - 1) // 2
                    y = self.Points[-1].y + height * (self.EndPoint[1][1] - 1)
                    dc.DrawText(self.ComputedValue, x, y)
                else:
                    middle = len(self.Segments) // 2 + \
                        len(self.Segments) % 2 - 1
                    x = (self.Points[middle].x +
                         self.Points[middle + 1].x - width) // 2
                    if self.BoundingBox[3] > height and self.Segments[middle] in [NORTH, SOUTH]:
                        y = (self.Points[middle].y +
...
class Comment(Graphic_Element):
    ...
    def DrawHighlightment(self, dc):
        ...
        polygon = [wxpatch.Point(left, top), wxpatch.Point(angle_top, top),
                   wxpatch.Point(right, angle_right), wxpatch.Point(
                       right, bottom),
                   wxpatch.Point(left, bottom)]
    ...
    def Draw(self, dc):
        ...
        polygon = [wxpatch.Point(self.Pos.x, self.Pos.y),
                   wxpatch.Point(self.Pos.x + self.Size[0] - 10, self.Pos.y),
                   wxpatch.Point(self.Pos.x + self.Size[0], self.Pos.y + 10),
                   wxpatch.Point(
                       self.Pos.x + self.Size[0], self.Pos.y + self.Size[1]),
                   wxpatch.Point(self.Pos.x, self.Pos.y + self.Size[1])]
        dc.DrawPolygon(polygon)

        # dc.SetBrush call is workaround for the issue with wx.PrinterDC
        # with wxPython 3.0 on GNU/Linux (don't remove it)
        dc.SetBrush(wx.WHITE_BRUSH)
        lines = [wxpatch.Point(self.Pos.x + self.Size[0] - 10, self.Pos.y),
                 wxpatch.Point(
                     self.Pos.x + self.Size[0] - 10, self.Pos.y + 10),
                 wxpatch.Point(self.Pos.x + self.Size[0], self.Pos.y + 10)]
        dc.DrawLines(lines)
        ...
        for idx, line in enumerate(self.Content.splitlines()):
            ...
            words = line.split(" ")
            if not getattr(dc, "printing", False):
                highlights = FilterHighlightsByRow(
                    self.Highlights, idx, len(line))
                highlights_offset = 0
            for i, word in enumerate(words):
                ...
                if wordwidth < self.Size[0] - 20:
                    if i < len(words) - 1:
                        ...
                        if not getattr(dc, "printing", False):
                            DrawHighlightedText(dc, text, FilterHighlightsByColumn(
                                highlights, highlights_offset, highlights_offset + len(text)), self.Pos.x + 10, y)
                    ...
                    if not first:
                        ...
                        if not getattr(dc, "printing", False):
                            DrawHighlightedText(dc, linetext, FilterHighlightsByColumn(
                                highlights, highlights_offset, highlights_offset + len(linetext)), self.Pos.x + 10, y)
                    ...
                    if first or i == len(words) - 1:
                        ...
                        if not getattr(dc, "printing", False):
                            DrawHighlightedText(dc, word, FilterHighlightsByColumn(
                                highlights, highlights_offset, highlights_offset + len(word)), self.Pos.x + 10, y)
