...
] = map(int, wx.NewIdRef(4))


class SearchResultPanel(wx.Panel):

    def _init_coll_MainSizer_Items(self, parent: wx.FlexGridSizer):
        parent.Add(self.HeaderSizer, 0, border=0, flag=wx.GROW)
        parent.Add(self.SearchResultsTree, 1, border=0, flag=wx.GROW)

    def _init_coll_MainSizer_Growables(self, parent: wx.FlexGridSizer):
        parent.AddGrowableCol(0)
        parent.AddGrowableRow(1)

    def _init_coll_HeaderSizer_Items(self, parent: wx.BoxSizer):
        parent.Add(self.HeaderLabel, 1, border=5, flag=wx.LEFT |
                   wx.RIGHT | wx.ALIGN_CENTER_VERTICAL)
        parent.Add(self.ResetButton, 0, border=0, flag=0)

    def _init_coll_HeaderSizer_Growables(self, parent: wx.FlexGridSizer):
        parent.AddGrowableCol(0)
    ...
    def _init_ctrls(self, prnt):
        self.HeaderLabel = wx.StaticText(id=ID_SEARCHRESULTPANELHEADERLABEL,
                                         ...
                                         pos=wxpatch.Point(0, 0), size=wx.Size(0, 17), style=0)
        ...
        self.SearchResultsTree = CT.CustomTreeCtrl(id=ID_SEARCHRESULTPANELSEARCHRESULTSTREE,
                                                   ...
                                                   pos=wxpatch.Point(0, 0), style=search_results_tree_style)
        if wx.VERSION >= (2, 8, 11):
            self.SearchResultsTree.SetAGWWindowStyleFlag(
        ...
        self.ResetButton = wx.BitmapButton(parent=self,
    ...
    def __init__(self, parent, window):
        super().__init__(id=ID_SEARCHRESULTPANEL,
                          name='SearchResultPanel', parent=parent,
                          pos=wxpatch.Point(0, 0),
        ...
        if self.Criteria is None:
            ...
            for tagname in self.ElementsOrder:
                ...
                if element_type == ITEM_POU:
                    element_type = self.ParentWindow.Controler.GetPouType(
                        words[1])
                    ...
                    if len(words) == 1:  # CTN match
                        ...
                        if child_type == "name":
                            ...
                            if element_type == ITEM_TRANSITION:
                                child_type = self.ParentWindow.Controler.GetTransitionBodyType(
                                    words[1], words[2])
                            elif element_type == ITEM_ACTION:
                                child_type = self.ParentWindow.Controler.GetActionBodyType(
                                    words[1], words[2])
                            else:
            ...
            self.HeaderLabel.SetLabel(header_format.format(
            ...
            if matches_number > 0:
                ...
                self.GenerateSearchResultsTreeBranch(
                    root, search_results_tree_infos)
    ...
    def GenerateSearchResultsTreeBranch(self, root, infos):
        ...
        if infos["type"] is not None:
            if infos["type"] == ITEM_POU:
                self.SearchResultsTree.SetItemImage(
                    root, self.TreeImageDict[self.ParentWindow.Controler.GetPouType(infos["name"])])
            else:
                self.SearchResultsTree.SetItemImage(
        ...
        if infos["text"] is not None:
            ...
            start_idx = start[1]
            end_idx = reduce(lambda x, y: x + y, map(lambda x: len(x) +
                             1, text_lines[:end[0] - start[0]]), end[1] + 1)
        ...
        if text is not None:
            ...
            text_ctrl = wx.TextCtrl(id=-1, parent=self.SearchResultsTree, pos=wxpatch.Point(0, 0),
            ...
            text_ctrl.SetBackgroundColour(
                self.SearchResultsTree.GetBackgroundColour())
            text_ctrl.Bind(wx.EVT_LEFT_DOWN,
                           self.GetTextCtrlClickFunction(root))
            text_ctrl.Bind(wx.EVT_LEFT_DCLICK,
                           self.GetTextCtrlDClickFunction(root))
