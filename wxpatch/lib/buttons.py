"""
This module fix the float param error in 'wxpatch.buttons'
"""
...
class GenButton(wx.Control):
    ...
    def __init__(self, parent, id=-1, label='',
        ...
        :param `pos`: the control position. A value of (-1, -1) indicates a default
         ...
         platform;
        :type `pos`: tuple or :class:`wxpatch.Point`
    ...
    def DrawBezel(self, dc, x1, y1, x2, y2):
        ...
        for i in range(self.bezelWidth):
            wxpatch.DrawLine(dc, x1+i, y1, x1+i, y2-i)
            wxpatch.DrawLine(dc, x1, y1+i, x2-i, y1+i)
        ...
        if self.up:
            ...
            dc.SetPen(wx.Pen(self.highlightPenClr, 1))
        for i in range(self.bezelWidth):
            wxpatch.DrawLine(dc, x1+i, y2-i, x2+1, y2-i)
            wxpatch.DrawLine(dc, x2-i, y1+i, x2-i, y2)


    def DrawLabel(self, dc: wx.DC, width: int, height: int, dx=0, dy=0):
    ...
    def DrawFocusIndicator(self, dc: wx.DC, w: int, h: int):
        bw = self.bezelWidth
        ...
        wxpatch.DrawRectangle(dc, bw+2,bw+2,  w-bw*2-4, h-bw*2-4)
    ...
    def GetBackgroundBrush(self, dc: wx.DC):
        """
    ...
    def OnGainFocus(self, event: wx.FocusEvent):
        """
    ...
    def OnLoseFocus(self, event: wx.FocusEvent):
        """
    ...
    def OnLoseCapture(self, event: wx.MouseCaptureChangedEvent):
        """
...
class GenBitmapTextButton(GenBitmapButton):
    ...
    def __init__(self, parent, id=-1, bitmap=wx.NullBitmap, label='',
        ...
        super().__init__(parent, id, bitmap, pos, size, style, validator, name)
        self.SetLabel(label)
...
class __ThemedMixin(wx.EvtHandler):
    ...
    def DrawBezel(self, dc: wx.DC, x1: int, y1: int, x2: int, y2: int):

        rect = wxpatch.Rect(x1, y1, x2, y2)
        if self.up:
            state = 0
        else:
