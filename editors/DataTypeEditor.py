...
def AppendMenu(parent: wx.Menu, id: int, item, helpString, kind):
    return parent.Append(id=id, item=item, helpString=helpString, kind=kind)
...
class DataTypeEditor(EditorPanel):
    ...
    def _init_Editor(self, parent):
        ...
        derivation_type_label = wx.StaticText(
            self.Editor, label=_('Derivation Type:'))
        ...
        self.Bind(wx.EVT_COMBOBOX, self.OnDerivationTypeChanged,
                  self.DerivationType)
        top_sizer.Add(self.DerivationType, border=5,
        ...
        self.DirectlyBaseType = wx.ComboBox(
            self.DirectlyPanel, style=wx.CB_READONLY)
        ...
        self.SubrangeBaseType = wx.ComboBox(
            self.SubrangePanel, style=wx.CB_READONLY)
        ...
        self.SubrangeInitialValue = CustomIntCtrl(
            self.SubrangePanel, style=wx.TAB_TRAVERSAL)
        self.SubrangeInitialValue.Bind(
            CustomIntCtrl.EVT_CUSTOM_INT, self.OnInfosChanged)
        subrange_panel_sizer.Add(self.SubrangeInitialValue, 1, border=5,
                                 flag=wx.GROW | wx.ALL)

        subrange_minimum_label = wx.StaticText(
            self.SubrangePanel, label=_('Minimum:'))
        subrange_panel_sizer.Add(subrange_minimum_label, 1, border=5,
                                 flag=wx.ALIGN_CENTER_VERTICAL | wx.ALL)

        self.SubrangeMinimum = CustomIntCtrl(
            self.SubrangePanel, style=wx.TAB_TRAVERSAL)
        self.SubrangeMinimum.Bind(
            CustomIntCtrl.EVT_CUSTOM_INT, self.OnSubrangeMinimumChanged)
        ...
        self.SubrangeMaximum = CustomIntCtrl(
            self.SubrangePanel, style=wx.TAB_TRAVERSAL)
        self.SubrangeMaximum.Bind(
            CustomIntCtrl.EVT_CUSTOM_INT, self.OnSubrangeMaximumChanged)
        ...
        array_basetype_label = wx.StaticText(
            self.ArrayPanel, label=_('Base Type:'))
        ...
        structure_panel_sizer = wx.FlexGridSizer(
            cols=1, hgap=0, rows=2, vgap=0)
        structure_panel_sizer.AddGrowableCol(0)
        structure_panel_sizer.AddGrowableRow(1)

        structure_button_sizer = wx.FlexGridSizer(
            cols=5, hgap=5, rows=1, vgap=0)
        ...
        structure_elements_label = wx.StaticText(self.StructurePanel,
                                                 label=_('Elements :'))
        structure_button_sizer.Add(
            structure_elements_label, flag=wx.ALIGN_BOTTOM)
        ...
        for name, bitmap, help in [
            ...
            button = wx.BitmapButton(parent=self.StructurePanel,
        ...
        structure_panel_sizer.Add(
            self.StructureElementsGrid, flag=wx.GROW)
    ...
    def __init__(self, parent, tagname, window, controler):
        super().__init__(parent, tagname, window, controler)

        self.StructureElementDefaultValue = {
            "Name": "", "Type": DefaultType, "Initial Value": ""}
        self.StructureElementsTable = ElementsTable(
            self, [], GetElementsTableColnames())
        ...
        self.DATATYPE_TYPES_DICT = dict(
            [(_(datatype), datatype) for datatype in GetDatatypeTypes()])
        ...
        def _DeleteStructureElement(row):
            ...
            self.RefreshTypeInfos()
            self.StructureElementsTable.ResetView(self.StructureElementsGrid)
        setattr(self.StructureElementsGrid,
                "_DeleteRow", _DeleteStructureElement)
        ...
        for col in range(self.StructureElementsTable.GetNumberCols()):
            ...
            attr.SetAlignment(
                self.StructureColAlignements[col], wx.ALIGN_CENTRE)
        ...
        if type_infos is not None:
            ...
            if type_infos["type"] == "Directly":
                ...
                self.ArrayDimensions.SetStrings(
                    map("..".join, type_infos["dimensions"]))
    ...
    def OnSubrangeMinimumChanged(self, event):
        if not self.Initializing:
            wx.CallAfter(self.SubrangeMinimum.SetValue, min(
                self.SubrangeMaximum.GetValue(), self.SubrangeMinimum.GetValue()))
    ...
    def OnSubrangeMaximumChanged(self, event):
        if not self.Initializing:
            wx.CallAfter(self.SubrangeMaximum.SetValue, max(
                self.SubrangeMinimum.GetValue(), self.SubrangeMaximum.GetValue()))
            wx.CallAfter(self.RefreshSubrangeInitialValueRange)
            wx.CallAfter(self.RefreshTypeInfos)
        event.Skip()
    ...
    def OnEnumeratedValueEndEdit(self, event):
        ...
        if index >= len(values) or values[index].upper() != text.upper():
            if text.upper() in [value.upper() for value in values]:
                message = wx.MessageDialog(self, _("\"%s\" value already defined!") % text, _(
                ...
                message = wx.MessageDialog(self, _(
                    "\"%s\" is a keyword. It can't be used!") % text, _("Error"), wx.OK | wx.ICON_ERROR)
                message.ShowModal()
                message.Destroy()
            else:
...
#                message = _("A pou with \"%s\" as name exists!")%value
                ...
                message = _(
                    "An element named \"%s\" already exists in this structure!") % value
            else:
                self.RefreshTypeInfos()
    ...
    def OnStructureElementsGridEditorShown(self, event):
        ...
        if self.StructureElementsTable.GetColLabelValue(col, False) == "Type":
            ...
            for base_type in self.Controler.GetBaseTypes():
                item: wx.MenuItem = AppendMenu(base_menu,
                                               help='',
                                               id=wx.ID_ANY,
                                               kind=wx.ITEM_NORMAL,
                                               text=base_type)

                self.Bind(wx.EVT_MENU, self.GetElementTypeFunction(
                    base_type), id=item.GetId())
            type_menu.AppendMenu(wx.ID_ANY, _("Base Types"), base_menu)

            datatype_menu = wx.Menu(title='')
            for datatype in self.Controler.GetDataTypes(self.TagName, False):
                item: wx.MenuItem = AppendMenu(datatype_menu,
                                               help='',
                                               id=wx.ID_ANY,
                                               kind=wx.ITEM_NORMAL,
                                               text=datatype)
                self.Bind(wx.EVT_MENU, self.GetElementTypeFunction(
                    datatype), id=item.GetId())
            type_menu.AppendMenu(wx.ID_ANY, _(
                "User Data Types"), datatype_menu)

            item: wx.MenuItem = AppendMenu(type_menu,
                                           help='',
                                           id=wx.ID_ANY,
                                           kind=wx.ITEM_NORMAL,
                                           text=_("Array"))

            self.Bind(wx.EVT_MENU, self.ElementArrayTypeFunction,
                      id=item.GetId())
...
#                type_menu.AppendMenu(wx.NewId(), _("Function Block Types"), functionblock_menu)
            ...
            rect = self.StructureElementsGrid.BlockToDeviceRect(
                (row, col), (row, col))
            self.StructureElementsGrid.PopupMenu(type_menu,
                                                 rect.x + rect.width,
    ...
    def ElementArrayTypeFunction(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            self.StructureElementsTable.SetValueByName(
                row, "Type", dialog.GetValue())
            self.RefreshTypeInfos()
            self.StructureElementsTable.ResetView(self.StructureElementsGrid)
        dialog.Destroy()
    ...
    def RefreshBoundsRange(self):
        ...
        if range is not None:
            ...
            self.SubrangeMinimum.SetValue(
                min(max(min_value, self.SubrangeMinimum.GetValue()), max_value))
            self.SubrangeMaximum.SetBounds(min_value, max_value)
            self.SubrangeMaximum.SetValue(
                min(max(min_value, self.SubrangeMaximum.GetValue()), max_value))

    def RefreshSubrangeInitialValueRange(self):
        self.SubrangeInitialValue.SetBounds(
            self.SubrangeMinimum.GetValue(), self.SubrangeMaximum.GetValue())
    ...
    def RefreshTypeInfos(self):
        ...
        if selected == "Directly":
            ...
            for dimensions in self.ArrayDimensions.GetStrings():
                ...
                if result is None:
                    message = wx.MessageDialog(self, _(
                        "\"%s\" value isn't a valid array dimension!") % dimensions, _("Error"), wx.OK | wx.ICON_ERROR)
                    message.ShowModal()
                    message.Destroy()
                    self.RefreshView()
                    return
            ...
            if infos[0] == "struct":
                ...
                if control is not None:
                    if isinstance(control, (wx.ComboBox, wx.SpinCtrl)):
                        ...
                        control.SetStyle(
                            start[1], end[1] + 1, wx.TextAttr(highlight_type[1], highlight_type[0]))
                    elif isinstance(control, wx.gizmos.EditableListBox):
                        listctrl = control.GetListCtrl()
