...
class GenBitmapTextButton(wxpatch.lib.buttons.GenBitmapTextButton):
    ...
    def DrawLabel(self, dc: wx.DC, width: int, height: int, dw=0, dy=0):
        bmp = self.bmpLabel
        ...
        if bmp is not None:
            ...
            dc.DrawBitmap(bmp, pos_x, pos_y, hasMask)
            # adjust for bitmap and text to centre
            pos_x = (width-tw)//2+dw
            pos_y += bh + 2
...
class ConfTreeNodeEditor(EditorPanel):
    ...
    def _init_Editor(self, parent):
        ...
        if tabs_num > 1 or self.SHOW_BASE_PARAMS:
            ...
            if self.SHOW_BASE_PARAMS:
                ...
                self.IECCUpButton = wxpatch.lib.buttons.GenBitmapTextButton(
                    self.Editor,
                ...
                self.IECCDownButton = wx.BitmapButton(parent=self.Editor,
                                                      bitmap=GetBitmap(
                                                          'IECCUp'),
                                                      size=wx.Size(16, 16),
                                                      style=wx.NO_BORDER)
                ...
                self.ConfNodeName = wx.TextCtrl(self.Editor,
                                                size=wx.Size(150, 25))
                self.ConfNodeName.SetFont(wx.Font(faces["size"] * 0.75,
                                                  wx.DEFAULT,
                                                  wx.NORMAL,
                                                  wx.BOLD,
                                                  faceName=faces["helv"]))
                self.ConfNodeName.Bind(
                    wx.EVT_TEXT,
                    self.GetTextCtrlCallBackFunction(
                        self.ConfNodeName, "BaseParams.Name", True),
                    self.ConfNodeName)
                baseparamseditor_sizer.Add(self.ConfNodeName,
                                           border=5,
                                           flag=wx.LEFT | wx.RIGHT | wx.ALIGN_CENTER_VERTICAL)

                buttons_sizer = self.GenerateMethodButtonSizer()
                baseparamseditor_sizer.Add(
                    buttons_sizer, flag=wx.ALIGN_CENTER)

            if tabs_num > 1:
                self.ConfNodeNoteBook = wx.Notebook(self.Editor)
                parent = self.ConfNodeNoteBook
                self.MainSizer.Add(
                    self.ConfNodeNoteBook, 1, flag=wx.GROW)
            else:
                parent = self.Editor
        ...
        if self.SHOW_PARAMS and len(self.Controler.GetParamsAttributes()) > 0:
            ...
            self.ParamsEditorSizer = wx.FlexGridSizer(
                cols=1, hgap=0, rows=1, vgap=5)
    ...
    def RefreshIECChannelControlsState(self):
        ...
        self.IECCDownButton.Enable(
            self.Controler.BaseParams.getIEC_Channel() > 0)
    ...
    def RefreshConfNodeParamsSizer(self):
        ...
        if len(confnode_infos) > 0:
            self.GenerateSizerElements(
                self.ConfNodeParamsSizer, confnode_infos, None, False)

        self.ParamsEditorSizer.Layout()
        self.Thaw()

    def GenerateMethodButtonSizer(self):
        normal_bt_font = wx.Font(
            faces["size"] // 3,    wx.DEFAULT, wx.NORMAL, wx.NORMAL, faceName=faces["helv"])
        mouseover_bt_font = wx.Font(
            faces["size"] // 3, wx.DEFAULT, wx.NORMAL, wx.NORMAL, faceName=faces["helv"], underline=True)
        ...
        for confnode_method in self.Controler.ConfNodeMethods:
            if "method" in confnode_method and confnode_method.get("shown", True):
                ...
                if confnode_method.get("push", False):
                    button.Bind(wx.EVT_LEFT_DOWN, self.GetButtonCallBackFunction(
                        confnode_method["method"], True))
                else:
                    button.Bind(wx.EVT_BUTTON, self.GetButtonCallBackFunction(
                        confnode_method["method"]), button)
                ...
                def setFontStyle(b, s):
                    def fn(event):
                        ...
                        event.Skip()
                    return fn
                button.Bind(wx.EVT_ENTER_WINDOW, setFontStyle(
                    button, mouseover_bt_font))
                button.Bind(wx.EVT_LEAVE_WINDOW,
                            setFontStyle(button, normal_bt_font))
        ...
        for element_infos in elements:
            ...
            if element_infos["type"] == "element":
                ...
                staticbitmap = GenStaticBitmap(
                    ...
                    pos=wxpatch.Point(0, 0), size=wx.Size(24, 24), style=0)
                ...
                if isinstance(element_infos["type"], list):
                    if isinstance(element_infos["value"], tuple):
                        ...
                        if len(element_infos["type"]) > 0 and isinstance(element_infos["type"][0], tuple):
                            ...
                            sizer.Add(
                                staticboxsizer, border=5, flag=wx.GROW | wx.BOTTOM | wx.LEFT | wx.RIGHT)
                            self.GenerateSizerElements(
                                staticboxsizer, element_infos["children"], element_path)
                            callback = self.GetChoiceContentCallBackFunction(
                                combobox, staticboxsizer, element_path)
                        else:
                            for choice in element_infos["type"]:
                                combobox.Append(choice)
                            callback = self.GetChoiceCallBackFunction(
                                combobox, element_path)
                    ...
                    spinctrl.Bind(wx.EVT_SPINCTRL,
                                  self.GetTextCtrlCallBackFunction(
                                      spinctrl, element_path),
                                  spinctrl)

                else:
                    if element_infos["type"] == "boolean":
                        checkbox = wx.CheckBox(
                            self.ParamsEditor, size=wx.Size(17, 25))
                        boxsizer.Add(checkbox)
                        if element_infos["value"] is not None:
                            checkbox.SetValue(element_infos["value"])
                        checkbox.Bind(wx.EVT_CHECKBOX,
                                      self.GetCheckBoxCallBackFunction(
                                          checkbox, element_path),
                                      checkbox)
                        ...
                        choices = self.ParentWindow.GetConfigEntry(
                            element_path, [""])
                        ...
                        if element_infos["name"] == "URI_location":
                            uriSizer = wx.FlexGridSizer(
                                cols=2, hgap=0, rows=1, vgap=0)
                            uriSizer.AddGrowableCol(0)
                            uriSizer.AddGrowableRow(0)

                            self.EditButton = wx.Button(
                                self.ParamsEditor, label='...', size=wx.Size(30, -1))
                            self.Bind(wx.EVT_BUTTON, self.UriOptions,
                                      self.EditButton)
                        ...
                        callback = self.GetTextCtrlCallBackFunction(
                            textctrl, element_path)
    ...
    def GetItemChannelChangedFunction(self, dir):
        def OnConfNodeTreeItemChannelChanged(event):
            ...
            self.SetConfNodeParamsAttribute(
                "BaseParams.IEC_Channel", confnode_IECChannel + dir)
            wx.CallAfter(self.RefreshIECChannelControlsState)
            wx.CallAfter(self.ParentWindow._Refresh,
                         TITLE, FILEMENU, PROJECTTREE)
            event.Skip()
    ...
    def GetTextCtrlCallBackFunction(self, textctrl, path, refresh=False):
        def OnTextCtrlChanged(event):
            ...
            if refresh:
                wx.CallAfter(self.ParentWindow._Refresh, TITLE,
                             FILEMENU, PROJECTTREE, PAGETITLES)
                wx.CallAfter(
                    self.ParentWindow.SelectProjectTreeItem, self.GetTagName())
            event.Skip()
    ...
    def GetBrowseCallBackFunction(self, name, textctrl, library, value_infos, path):
        ...
        def OnBrowseButton(event):
            ...
            if dialog.ShowModal() == wx.ID_OK:
                value, value_infos = self.SetConfNodeParamsAttribute(
                    path, dialog.GetValueInfos())
